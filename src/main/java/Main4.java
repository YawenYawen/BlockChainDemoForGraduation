...
public class Main4 {
    ...
    public static final String GROUP_IP = "239.193.129.14";//224.0.0.0 ~ 239.255.255.255
    public static final Integer GROUP_PORT = 9876;
        ...
        new Thread(new Runnable() {
            public void run() {
                try {
                    ...
                    while (true) {
//                        send(socket, group, getTitle(socket) + "Main3");
    ...
    public static void receive(MulticastSocket socket, InetAddress group) throws Exception {
        ...
        System.out.println(getTitle(socket) + "^^^接收数据:" + new String(buf, 0, datagramPacket.getLength()));
    }
